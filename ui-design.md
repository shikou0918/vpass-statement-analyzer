...
Vue Router で画面ごとにURLを持たせる。`/` は初期導線として `/imports/new` にリダイレクトし、各画面URLを直接開いた場合やリロードした場合は同じ画面を表示する。
...
| KPI | 支出合計、前月比、明細件数 |
| 月別支出推移 | 請求月ごとの支出額を Chart.js の折れ線グラフで表示 |
| 日別推移 | 折れ線または棒グラフ |
| カテゴリ内訳 | ドーナツまたは横棒グラフ |
| 利用先ランキング | 上位利用先テーブル |
| 導線 | 明細一覧で見る、分析へ移動 |
...
| 一括カテゴリ変更 | チェックした明細に同じカテゴリをまとめて適用。未分類への一括変更も可能 |
| メモ変更 | 詳細ドロワーまたはインライン編集 |
| 分析除外 | `excludedFromAnalytics` を切り替える |

カテゴリ変更は optimistic update 可。ただし失敗時は元の値へ戻し、行内または画面上部にエラーを表示する。一括カテゴリ変更は選択件数が0件の場合 disabled にする。
